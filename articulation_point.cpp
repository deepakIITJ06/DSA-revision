...
// 🔴🔴🔴🔴🔴🔴🔴 LOGIC 🔴🔴🔴🔴🔴🔴🔴
// simply time of reaching store kr rahein hein --> along with ki minimum time kya ho sakta hai apart from parent (means ki agar child parent hai then--> continue BHULNA NAHI HAI ye case) and 🔴 visited node too
...
// then after coming back to that node --> check ki is node tak aane tak ka time se <= (large) to nahin iske child tak aane ka time if "YES" --> that means ki wo ek ARTICULATION point hai
